...
export const TenantTicketsView: React.FC<TenantTicketsViewProps> = ({ tickets, property, tenant, addTicket }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="primary" onClick={() => setIsCreateModalOpen(true)}>+ Crear Nuevo Ticket</Button>
      </div>
        ...
        {tickets.map(ticket => (
          <Card key={ticket.id} className="flex flex-col">
            <div className="p-4">
              ...
              <p className="text-sm text-gray-500 mt-1">Reportado: {formatDate(ticket.fecha_creacion)}</p>
            </div>
      ...
      {selectedTicket && (
        <Modal isOpen={!!selectedTicket} onClose={() => setSelectedTicket(null)} title={selectedTicket.titulo} size="2xl">
            <div className="space-y-4">
                <div>
                    <h4 className="font-semibold text-gray-800 mb-1">Estado</h4>
                    <Badge text={selectedTicket.estado} color={getStatusBadgeColor(selectedTicket.estado)} />
                </div>
                <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {selectedTicket.fotos.map((foto, index) => (
                                <a key={index} href={foto} target="_blank" rel="noopener noreferrer">
                                    <img src={foto} alt={`Foto ${index + 1}`} className="w-full h-32 object-cover rounded-md shadow-sm" />
                                </a>
                            ))}
                        </div>
                    </div>
