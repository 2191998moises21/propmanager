...
export const UploadPaymentModal: React.FC<UploadPaymentModalProps> = ({
  ...
  const month = new Date(payment.mes_correspondiente).toLocaleDateString('es-ES', {
    month: 'long',
    year: 'numeric',
  });
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={`Subir Comprobante de Pago - ${month}`}
      size="lg"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <p className="text-sm text-gray-700 mb-2">
            Por favor, suba el comprobante de pago para el mes de{' '}
            <span className="font-bold capitalize">{month}</span> por un monto de{' '}
