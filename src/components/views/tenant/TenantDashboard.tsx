...
export const TenantDashboard: React.FC<TenantDashboardProps> = ({ tenant, tickets, payments, setView }) => {
    const nextPayment = useMemo(() => {
        return payments
            .filter(p => p.estado_pago === PaymentStatus.Pendiente || p.estado_pago === PaymentStatus.Atrasado)
            .sort((a,b) => new Date(a.mes_correspondiente).getTime() - new Date(b.mes_correspondiente).getTime())[0];
    }, [payments]);

    const openTicketsCount = useMemo(() => {
        return tickets.filter(t => t.estado === TicketStatus.Abierto || t.estado === TicketStatus.EnProgreso).length;
    }, [tickets]);

    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-gray-900">Bienvenido, {tenant.nombre_completo.split(' ')[0]}</h1>
                    ...
                    <div>
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                {nextPayment ? (
                                    <>
                                        <p className="text-2xl font-semibold text-gray-900">{formatCurrency(nextPayment.monto_pago)}</p>
                                        <p className="text-sm text-gray-500 capitalize">Para {formatMonth(nextPayment.mes_correspondiente)}</p>
                                    </>
                                ) : (
            ...
            <Card>
                <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4">
                    <div className="flex items-center">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800">¿Tienes un problema en tu propiedad?</h3>
                            <p className="text-gray-600">No dudes en reportarlo. Crea un ticket de mantenimiento y nos encargaremos.</p>
                        </div>
                    </div>
