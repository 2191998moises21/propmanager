...
export const TenantPaymentsView: React.FC<TenantPaymentsViewProps> = ({
  payments,
  contract,
  uploadPaymentProof,
}) => {
  ...
  const sortedPayments = [...payments].sort(
    (a, b) => new Date(b.mes_correspondiente).getTime() - new Date(a.mes_correspondiente).getTime()
  );
              ...
              {sortedPayments.map((payment) => (
                <tr key={payment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {(payment.estado_pago === PaymentStatus.Pendiente ||
                      payment.estado_pago === PaymentStatus.Atrasado) && (
                      <Button
                        variant="primary"
                        size="sm"
