...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const occupiedProperties = properties.filter(
      (p) => p.estado_ocupacion === OccupancyStatus.Ocupada
    ).length;
  ...
  const pendingPayments = useMemo(
    () =>
      payments
        .filter(
          (p) =>
            p.estado_pago === PaymentStatus.Pendiente || p.estado_pago === PaymentStatus.Atrasado
        )
        .slice(0, 5),
    [payments]
  );
          ...
          <div className="space-y-4">
            {recentProperties.map((prop) => (
              <button
                ...
                className="w-full text-left flex items-center justify-between p-2 rounded-md hover:bg-gray-50 transition-colors"
              >
                ...
                <Badge
                  text={prop.estado_ocupacion}
                  color={prop.estado_ocupacion === 'disponible' ? 'green' : 'blue'}
                />
              </button>
            ))}
          </div>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Vence:{' '}
                    {new Date(payment.mes_correspondiente).toLocaleDateString('es-ES', {
                      month: 'long',
                      year: 'numeric',
                    })}
                  </p>
                ...
                <Badge
                  ...
                  color={payment.estado_pago === 'pendiente' ? 'yellow' : 'red'}
                />
              </div>
            ))}
