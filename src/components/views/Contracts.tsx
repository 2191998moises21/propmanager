...
export const Contracts: React.FC<ContractsProps> = ({ contracts, properties, tenants, addContract, onSelectContract }) => {
    ...
    return (
      <div className="bg-white p-8 rounded-lg shadow-md max-w-4xl mx-auto">
        ...
        <form onSubmit={handleAddContract} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="propertyId" className="block text-sm font-medium text-gray-700">Propiedad</label>
            <select name="propertyId" id="propertyId" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm">
              <option value="">Seleccione una propiedad disponible</option>
              {availableProperties.map(prop => <option key={prop.id} value={prop.id}>{prop.title}</option>)}
            </select>
            {availableProperties.length === 0 && <p className="text-xs text-red-500 mt-1">No hay propiedades disponibles.</p>}
          </div>
          <div>
          ...
          <div>
            <label htmlFor="monto_mensual" className="block text-sm font-medium text-gray-700">Monto Mensual</label>
            <input type="number" name="monto_mensual" id="monto_mensual" placeholder="Monto en USD" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm" />
          </div>
          <div>
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 pt-4">
            ...
            <Button variant="primary" type="submit" disabled={availableProperties.length === 0}>Guardar Contrato</Button>
          </div>
        </form>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
       <div className="flex items-center justify-between">
            ...
            <Button variant="primary" onClick={() => setShowAddForm(true)}>+ Crear Contrato</Button>
        </div>
        <Card>
                       ...
                       return (
                        <tr key={contract.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                             ...
                             <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-semibold">{new Intl.NumberFormat('es-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }).format(contract.monto_mensual)}</td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <Badge text={contract.estado_contrato} color={getStatusBadgeColor(contract.estado_contrato)} />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button onClick={() => onSelectContract(contract.id)} className="text-primary hover:text-blue-700">Ver Detalles</button>
                            </td>
                        </tr>
                       )
                    })}
